#!/usr/bin/env python3
"""
Create Levtiades Atlas - Fixed Version
...
def find_common_space_levinson(levinson_data):
    """Find the common space for Levinson components (use LC as reference)"""
    
    print("\n🔧 Finding Common Space for Levinson Components...")
    
    # Use LC as reference (it's the largest)
    ref_img = levinson_data['LC']['img']
    print(f"   Using LC as reference: {ref_img.shape}, {ref_img.header.get_zooms()[:3]} mm")
    
    # Create combined mask in LC space
    combined_mask = np.zeros_like(levinson_data['LC']['data'])
    
    # Resample all other components to LC space
    ...
    for name, label in label_mapping.items():
        if name in levinson_data and name != 'GM_MASK':
            if name == 'LC':
                # LC is already in reference space
                mask = levinson_data[name]['data'] > 0
                combined_mask[mask] = label
                voxel_count = np.sum(mask)
            else:
                # Resample to LC space
                print(f"   Resampling {name} to LC space...")
                resampled = image.resample_to_img(
                    levinson_data[name]['img'], 
                    ref_img, 
...
def align_all_atlases(levinson_img, tian_img, des_img):
    """Align all atlases to a common space (using Tian as reference)"""
    
    print("\n🔄 Aligning All Atlases to Common Space...")
    
    aligned_dir = Path("levtiades_atlas/aligned_atlases")
    aligned_dir.mkdir(exist_ok=True)
    
    # Check current properties
    print("\n📊 Current Atlas Properties:")
    print(f"   Levinson: {levinson_img.shape}, {levinson_img.header.get_zooms()[:3]} mm")
    print(f"   Tian: {tian_img.shape}, {tian_img.header.get_zooms()[:3]} mm")
    print(f"   Destrieux: {des_img.shape}, {des_img.header.get_zooms()[:3]} mm")
    
    # Use Tian as reference (2mm resolution, good middle ground)
    print("\n   Using Tian as reference space (2mm resolution)...")
    ...
    # Resample Levinson (0.5mm -> 2mm)
    print("   Resampling Levinson (0.5mm -> 2mm)...")
    ...
    # Resample Destrieux to ensure alignment
    print("   Resampling Destrieux to ensure alignment...")
    ...
    print("✅ All atlases aligned to common 2mm MNI space")
...
def create_label_files(levinson_labels):
        ...
        with open(tian_label_file, 'r') as f:
            for line in f:
                if ':' in line and not line.startswith('#'):
                    ...
                    if len(parts) == 2:
                        try:
                            tian_labels[int(parts[0])] = parts[1].strip()
                        except ValueError:
                            continue
        ...
        with open(des_label_file, 'r') as f:
            for line in f:
                if ':' in line and not line.startswith('#'):
                    ...
                    if len(parts) == 2:
                        try:
                            des_labels[int(parts[0])] = parts[1].strip()
                        except ValueError:
                            continue
    ...
    with open(label_file, 'w') as f:
        ...
        for label, name in sorted(levinson_labels.items()):
...
def create_analysis_report(overlap_stats, changes, final_stats):
    ...
    with open(report_path, 'w') as f:
        ...
        if changes['destrieux_regions_affected']:
            ...
            for region, count in sorted(changes['destrieux_regions_affected'].items())[:10]:  # Top 10
                f.write(f"- Region {region}: {count} voxels\n")
...
def create_validation_plots(levinson_img, tian_img, des_img, combined_hier):
    """Create validation overlays for MRIcrogl"""
    
    print("\n🎨 Creating Validation Overlays...")
    
    vis_dir = Path("levtiades_atlas/validation")
    ...
    overlap_viz = np.zeros_like(levinson_data)
    overlap_viz[(levinson_data > 0) & (tian_data > 0)] = 1  # Levinson-Tian overlap
    overlap_viz[(levinson_data > 0) & (des_data > 0)] = 2   # Levinson-Destrieux overlap  
    overlap_viz[(tian_data > 0) & (des_data > 0)] = 3       # Tian-Destrieux overlap
    overlap_viz[(levinson_data > 0) & (tian_data > 0) & (des_data > 0)] = 4  # All three
    
    overlap_img = nib.Nifti1Image(overlap_viz.astype(np.uint8), levinson_img.affine, levinson_img.header)
    nib.save(overlap_img, vis_dir / "overlap_visualization.nii.gz")
    
    # Create individual atlas masks
    levinson_mask = (levinson_data > 0).astype(np.uint8) * 100
    tian_mask = (tian_data > 0).astype(np.uint8) * 150
    des_mask = (des_data > 0).astype(np.uint8) * 200
    
    nib.save(nib.Nifti1Image(levinson_mask, levinson_img.affine, levinson_img.header), 
             vis_dir / "levinson_mask.nii.gz")
    nib.save(nib.Nifti1Image(tian_mask, tian_img.affine, tian_img.header), 
             vis_dir / "tian_mask.nii.gz")
    nib.save(nib.Nifti1Image(des_mask, des_img.affine, des_img.header), 
             vis_dir / "destrieux_mask.nii.gz")
    
    print("✅ Validation overlays created")
...
if __name__ == "__main__":
    ...
    # Step 2: Create combined Levinson mask with proper resampling
    levinson_img, levinson_label_mapping = find_common_space_levinson(levinson_data)
    ...
    # Step 3: Align all atlases to common space
    levinson_aligned, tian_aligned, des_aligned = align_all_atlases(levinson_img, tian_img, des_img)
    
    # Step 4: Create version with overlaps
    combined_overlaps, overlap_stats = create_levtiades_with_overlaps(levinson_aligned, tian_aligned, des_aligned)
    
    # Step 5: Create hierarchical version (no overlaps)
    combined_hierarchical, changes, final_stats = create_levtiades_hierarchical(levinson_aligned, tian_aligned, des_aligned)
    
    # Step 6: Create label files
    ...
    # Step 8: Create validation overlays
    create_validation_plots(levinson_aligned, tian_aligned, des_aligned, combined_hierarchical)
